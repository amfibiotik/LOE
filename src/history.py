...
class ScheduleHistory:
    ...
    def get_latest(self, date: str) -> dict:
        """Отримує останній збережений графік"""
        if date in self.data and len(self.data[date]) > 0:
            return self.data[date][-1]["schedule"]
        return None
    
    def get_changes(self, date: str, new_schedule: dict) -> List[str]:
        """Порівнює новий графік з останнім збереженим"""
